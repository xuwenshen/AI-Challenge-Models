...
class TopKDecoder(torch.nn.Module):
    ...
    def forward(self, inputs=None, encoder_hidden=None, encoder_outputs=None, cnn_enc=None, function=F.log_softmax,
        ...
        inflated_cnn_enc = _inflate(cnn_enc, self.k, 0)
        ...
        for _ in range(0, max_length):
            ...
            log_softmax_output, hidden, _ = self.rnn.forward_step(input_var, hidden,
                                                                  inflated_encoder_outputs,inflated_cnn_enc, function=function)
            ...
            input_var = (candidates % self.V).view(batch_size * self.k, 1).cuda()
            ...
            if eos_indices.nonzero().dim() > 0:
                sequence_scores.data.masked_fill_(eos_indices.cpu(), -float('inf'))
    ...
    def _backtrack(self, nw_output, nw_hidden, predecessors, symbols, scores, b, hidden_size):
        ...
        while t >= 0:
            ...
            current_output = nw_output[t].index_select(0, t_predecessors.cuda())
            if lstm:
                current_hidden = tuple([h.index_select(1, t_predecessors.cuda()) for h in nw_hidden[t]])
            else:
                current_hidden = nw_hidden[t].index_select(1, t_predecessors.cuda())
            current_symbol = symbols[t].index_select(0, t_predecessors.cuda())
        ...
        output = [step.index_select(0, re_sorted_idx.cuda()).view(b, self.k, -1) for step in reversed(output)]
        p = [step.index_select(0, re_sorted_idx.cuda()).view(b, self.k, -1) for step in reversed(p)]
        if lstm:
            h_t = [tuple([h.index_select(1, re_sorted_idx.cuda()).view(-1, b, self.k, hidden_size) for h in step]) for step in reversed(h_t)]
            h_n = tuple([h.index_select(1, re_sorted_idx.data).view(-1, b, self.k, hidden_size) for h in h_n])
        else:
            h_t = [step.index_select(1, re_sorted_idx.cuda()).view(-1, b, self.k, hidden_size) for step in reversed(h_t)]
