...
bleuscore = BLEUScore()
model_dir = '/data/xuwenshen/ai_challenge/code/ibm_seq2seq/models.2/'
...
def train(lr, net, epoch, train_loader, valid_loader, transform, hyperparameters, batch_size):
    ...
    for iepoch in range(epoch):
        ...
        for (_, data) in enumerate(train_loader, 0):
            ...
            decoder_outputs, ret_dict = net(entext, zhgtruth, enlen, True, teacher_forcing_ratio)
            ...
            if global_steps % 50 == 0:
                ...
                for i in range(min(5, len(prediction[i]))):
                    print (prediction[i])
                    print (zhstr[i])
                    print ('-------------------\n')
            ...
            if global_steps % 200 == 0:
                ...
                for (_, data) in enumerate(valid_loader, 0):
                    ...
                    decoder_outputs, ret_dict = net(entext, None, enlen, True, 0)
                    length = ret_dict['length']
                    prediction = [0 for i in range(len(length))]
                    tmppre = [_.squeeze().cpu().data.tolist() for _ in ret_dict['sequence']]
                    tmppre = np.array(tmppre).transpose(1, 0)
...
if __name__ == '__main__':
    ...
    pre_trained = torch.load('./models.2/ratio-0.998501-loss-7.158012-score-0.346166-steps-15000-model.pkl') 
